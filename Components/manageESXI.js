...
export default class ManageESXI extends Component {
  componentWillMount() {
    this.setState(
      ...
      () => {
        this.getAllVMs(!this.state.response);
      }
    );
  }
  ...
  getAllVMs(refresh) {
    ...
    SSH.execute(this.state.sshConfig, "vim-cmd vmsvc/getallvms")
      .then(result => {
        result.splice(0, 1);
        let VMPromiseArray = [];
        result.map(VMdata => {
          VMPromiseArray.push(
            SSH.execute(
              ...
              "vim-cmd vmsvc/power.getstate " +
                VMdata.substr(0, VMdata.indexOf(" "))
            )
          );
        });
          ...
          for (let i = 0; i < pwrStateArr.length; i++) {
            ...
            if (pwrStateArr[i][1].includes("on")) {
              ...
              VMDataArray.push(
                <TouchableHighlight
                  key={i}
                  onPress={() => this.openVMDetails(vmName, vmSessionID)}
                >
                  <View style={[styles.card, styles.vmSelectionBtn]}>
                    <Text style={styles.vmName}>{vmName}</Text>
                    <TouchableHighlight
                      style={[styles.btn, styles.vmPowerBtnOn]}
                      onPress={() => this.powerOffVM(vmSessionID)}
                    >
              ...
              VMDataArray.push(
                ...
                >
                  <View style={[styles.card, styles.vmSelectionBtn]}>
                    <Text style={styles.vmName}>{vmName}</Text>
                    <TouchableHighlight
                      style={[styles.btn, styles.vmPowerBtnOff]}
                      onPress={() => this.powerOnVM(vmSessionID)}
                    >
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        <TouchableHighlight
          style={[styles.btn, styles.buttonBg]}
          onPress={() => this.getAllVMs(true)}
        >
          <Text style={styles.buttonText}>
            {this.state.response ? "Refresh VMs Status" : "List All VMs"}
          </Text>
        </TouchableHighlight>
      </View>
    );
